...
{
    public class GrabInventoryVH
    {
        private readonly ILogger<GrabInventoryVH> _logger;
        private readonly IConfiguration _configuration;
        private readonly string _connectionString;

        public GrabInventoryVH(ILogger<GrabInventoryVH> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get")] HttpRequestData req)
        {
            ...
            try
            {
                _logger.LogInformation("🚀 GrabInventoryVH function started - Request ID: {requestId}", Guid.NewGuid());
                ...
                if (format.ToLower() == "html")
                {
                    ...
                    // Cache stable inventory responses for 10 minutes
            ...
            {
                stopwatch.Stop();
                _logger.LogError(ex, "❌ Error in GrabInventoryVH function after {ms}ms - {errorType}: {message}", 
        ...
        private async Task<List<dynamic>> GetTableDataDynamic(string tableName)
        {
            ...
            try
            {
                ...
                if (hasTypeIdColumn)
                {
                    ...
                    _logger.LogInformation("📝 Executing filtered query: {query} (TypeID = 2)", query);
                }
                else
                {
                ...
                {
                    var filterMessage = hasTypeIdColumn ? " (filtered by TypeID = 2)" : "";
...
<html>
<head>
    <title>Dynamic Database Explorer VH - Debug Mode</title>
    <style>
        ...
        .vh-specific {{ background: #ffe6f0; padding: 15px; border-radius: 4px; margin: 20px 0; border-left: 4px solid #ff1744; }}
    </style>
</head>
<body>
    <div class='container'>
        <h1>🗄️ Dynamic Database Explorer VH - Debug Mode</h1>
        <div class='success'>✅ Database connection successful!</div>
        
        <div class='vh-specific'>
            <h2>🏷️ VH Function - TypeID = 2 Filter</h2>
            <p>This function specifically filters records where <strong>TypeID = 2</strong></p>
        </div>
        ...
        <div class='table-info'>
            ...
            <p><strong>Filter:</strong> TypeID = 2</p>
        ...
        <div class='api-info'>
            <h2>🔗 API Usage</h2>
            <ul>
                <li><strong>Default (Units, TypeID=2):</strong> <code>/api/GrabInventoryVH</code></li>
                <li><strong>Specific Table:</strong> <code>/api/GrabInventoryVH?table=TableName</code></li>
                <li><strong>With Schema:</strong> <code>/api/GrabInventoryVH?schema=true</code></li>
                <li><strong>HTML Format:</strong> <code>/api/GrabInventoryVH?format=html</code></li>
                <li><strong>Debug Mode:</strong> <code>/api/GrabInventoryVH?schema=true&format=html</code></li>
            </ul>
        </div>
